...
export const getAdminDashboard = async (req, res) => {
  try {
    ...
    const customizableProducts = await Product.countDocuments({
      isCustomizable: true,
    });

    // CUSTOM CAKE
    const customPending = await CustomCakeRequest.countDocuments({
      status: "pending",
    });
    const customQuoted = await CustomCakeRequest.countDocuments({
      status: "quoted",
    });
    const customCompleted = await CustomCakeRequest.countDocuments({
      status: "completed",
    });
